...
test('signs only URLs from the configured COS bucket', () => {
  const media = signer();
  const source = 'https://sidu-1234567890.cos.ap-guangzhou.myqcloud.com/USERS/u1/posts/a.jpg';
  const signed = media.signUrl(source);
  assert.match(signed, /^https:\/\/sidu-1234567890\.cos\.ap-guangzhou\.myqcloud\.com\/USERS\/u1\/posts\/a\.jpg\?/);
...
test('keeps signed URLs stable during the cache window', () => {
  const media = signer();
  const source = 'https://sidu-1234567890.cos.ap-guangzhou.myqcloud.com/USERS/u1/videos/a.mp4';
...
test('deep-signs media URLs inside objects, arrays, and JSON message content', () => {
  const media = signer();
  const stillUrl = 'https://sidu-1234567890.cos.ap-guangzhou.myqcloud.com/USERS/u1/live/a.jpg';
  const motionUrl = 'https://sidu-1234567890.cos.ap-guangzhou.myqcloud.com/USERS/u1/live/a.mp4';
...
test('is a no-op when COS signing is not configured', () => {
  ...
  const source = 'https://sidu-1234567890.cos.ap-guangzhou.myqcloud.com/a.jpg';
